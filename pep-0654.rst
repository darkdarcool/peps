...
* **Implementing better Task spawning API in asyncio**.
  The current asyncio API for spawning child Tasks -- the ``asyncio.gather()``
  function [1]_ -- has a few shortcomings, the most notable of which is
  how it handles exceptions.  A better API, inspired by the Trio's "Nursery"
  primitive [13]_, has been a long-awaited feature of asyncio, blocked on
  Python not having native language level support for exception groups.
